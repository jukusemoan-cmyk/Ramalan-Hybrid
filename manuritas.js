...
function hanacarakaValue(name) {
    ...
    let total = 0;
    const details = [];
    const elementCount = { Api: 0, Air: 0, Tanah: 0, Udara: 0 };
...
function analyzeNamePatterns(name) {
    ...
    // Tentukan vokal dominan
    let dominantVowel = 'A';
    let maxVowel = 0;
    for (let v in vowels) {
        if (vowels[v] > maxVowel) {
            maxVowel = vowels[v];
            dominantVowel = v;
        }
    }
...
function getWatakAsli(name, hanacarakaTotal, watakData) {
    ...
    // Pilih template berdasarkan kondisi (disederhanakan)
    const template = watakData.templates.find(t => {
        // Evaluasi condition string (sederhana)
...
/**
 * Memilih template Kelebihan Nama
 */
function getNameStrengths(patterns, kelebihanData) {
    if (!kelebihanData || !kelebihanData.templates) return [];
    
    const strengths = [];
    
    for (let key in patterns) {
        if (patterns[key] === true) {
            const matched = kelebihanData.templates.filter(t => 
...
/**
 * Memilih template Kekurangan Nama
 */
function getNameWeaknesses(patterns, kekuranganData) {
    if (!kekuranganData || !kekuranganData.templates) return [];
    
    const weaknesses = [];
    
    for (let key in patterns) {
        if (patterns[key] === true) {
            const matched = kekuranganData.templates.filter(t => 
...
function getSaranPenyempurnaan(patterns, weton, saranData) {
    ...
    // Tentukan masalah berdasarkan patterns
    if (patterns.no_u) {
        const matched = saranData.templates.filter(t => t.condition.includes('kurang_unsur_air'));
        suggestions.push(...matched);
    }
    if (patterns.excess_a) {
        const matched = saranData.templates.filter(t => t.condition.includes('kurang_unsur_tanah'));
        suggestions.push(...matched);
    }
    if (patterns.too_short) {
        const matched = saranData.templates.filter(t => t.condition.includes('terlalu_pendek'));
        suggestions.push(...matched);
    }
    if (patterns.too_long) {
        const matched = saranData.templates.filter(t => t.condition.includes('terlalu_panjang'));
        suggestions.push(...matched);
    }
...
/**
 * Memilih template Peringatan Dini Energetik
 */
...
async function calculateManuritas(fullName, weton, wuku, data) {
    ...
    const tugasHidup = getTugasHidup(hanacarakaResult.total, weton.neptu, data.tugasHidup);
    const watak = getWatakAsli(fullName, hanacarakaResult.total, data.watak);
    const titikLuka = getTitikLuka(hanacarakaResult.total, weton.neptu, data.titikLuka);
    const jalurRezeki = getJalurRezeki(hanacarakaResult.total, data.jalurRezeki);
    const kelebihan = getNameStrengths(patterns.patterns, data.kelebihan);
    const kekurangan = getNameWeaknesses(patterns.patterns, data.kekurangan);
    const saran = getSaranPenyempurnaan(patterns.patterns, weton, data.saran);
    const petaEnergi = getPetaEnergi(patterns.patterns, wuku.name, data.petaEnergi);
    const extreme = getExtremePatterns(hanacarakaResult.total, patterns.patterns, data.extreme);
