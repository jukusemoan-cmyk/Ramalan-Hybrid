...
// ================== FUNGSI loadNameOptions LENGKAP ==================
// Letakkan di file: js/manuritas-generator.js

// Cache untuk data JSON
let namaSelarasData = null;

// Fungsi untuk load data JSON
async function loadNamaSelarasData() {
  if (namaSelarasData) return namaSelarasData;
  
  try {
    const response = await fetch('../data/nama-selaras-lengkap.json');
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    namaSelarasData = await response.json();
    console.log('✅ 150 prefix + 150 suffix loaded');
    return namaSelarasData;
  } catch (error) {
    console.warn('⚠️ Gagal load JSON, menggunakan fallback:', error);
    // Fallback data minimal
    return {
      prefixes: [
        {"text": "Arjuna", "meaning": "Kesatria gagah berani", "element": "Api", "hanacaraka_value": 30, "era": "lawas"},
        {"text": "Dewi", "meaning": "Bidadari anggun", "element": "Air", "hanacaraka_value": 22, "era": "lawas"},
        {"text": "Bayu", "meaning": "Dewa angin", "element": "Udara", "hanacaraka_value": 24, "era": "lawas"},
        {"text": "Wira", "meaning": "Kesatria", "element": "Api", "hanacaraka_value": 26, "era": "lawas"},
        {"text": "Lestari", "meaning": "Abadi berkelanjutan", "element": "Kayu", "hanacaraka_value": 27, "era": "modern"},
        {"text": "Bima", "meaning": "Kesatria kuat", "element": "Tanah", "hanacaraka_value": 34, "era": "lawas"},
        {"text": "Sinta", "meaning": "Simbol kesetiaan", "element": "Tanah", "hanacaraka_value": 18, "era": "lawas"},
        {"text": "Rama", "meaning": "Raja bijaksana", "element": "Logam", "hanacaraka_value": 30, "era": "lawas"},
        {"text": "Kirana", "meaning": "Sinar cahaya", "element": "Api", "hanacaraka_value": 23, "era": "modern"},
        {"text": "Candra", "meaning": "Rembulan", "element": "Air", "hanacaraka_value": 22, "era": "lawas"}
      ],
      suffixes: [
        {"text": "Wicaksana", "meaning": "Bijaksana", "element": "Udara", "hanacaraka_value": 35, "era": "lawas"},
        {"text": "Samudra", "meaning": "Lautan luas", "element": "Air", "hanacaraka_value": 33, "era": "lawas"},
        {"text": "Wijaya", "meaning": "Kemenangan", "element": "Api", "hanacaraka_value": 30, "era": "lawas"},
        {"text": "Prasetya", "meaning": "Janji suci", "element": "Logam", "hanacaraka_value": 38, "era": "lawas"},
        {"text": "Utama", "meaning": "Terutama", "element": "Api", "hanacaraka_value": 29, "era": "lawas"},
        {"text": "Santosa", "meaning": "Kebahagiaan", "element": "Kayu", "hanacaraka_value": 34, "era": "lawas"},
        {"text": "Mahendra", "meaning": "Raja agung", "element": "Logam", "hanacaraka_value": 37, "era": "lawas"},
        {"text": "Baskara", "meaning": "Matahari", "element": "Api", "hanacaraka_value": 32, "era": "lawas"},
        {"text": "Ananta", "meaning": "Tak terbatas", "element": "Udara", "hanacaraka_value": 33, "era": "lawas"},
        {"text": "Dirgantara", "meaning": "Angkasa", "element": "Udara", "hanacaraka_value": 39, "era": "modern"}
      ]
    };
  }
}

// Fungsi hitung kekuatan nama
function calculateNameStrengthFromParts(prefixValue, suffixValue, wetonNeptu, wukuValue = 5) {
  const hanacarakaTotal = (prefixValue || 20) + (suffixValue || 20);
  const baseStrength = 45;
  const hanacarakaBonus = Math.min(35, Math.floor(hanacarakaTotal / 2.5));
  const wetonBonus = (wetonNeptu * 2) % 15;
  const wukuBonus = wukuValue || 5;
  
  return Math.min(100, baseStrength + hanacarakaBonus + wetonBonus + wukuBonus);
}

// Fungsi dapatkan label kekuatan
function getStrengthLabel(score) {
  if (score >= 90) return { label: 'SANGAT KUAT', stars: '⭐⭐⭐⭐⭐', class: 'sangat-kuat' };
  if (score >= 75) return { label: 'KUAT', stars: '⭐⭐⭐⭐', class: 'kuat' };
  if (score >= 60) return { label: 'CUKUP', stars: '⭐⭐⭐', class: 'cukup' };
...
// Fungsi dapatkan ikon elemen
function getElementIcon(element) {
  const icons = {
    'Api': '🔥', 'Air': '💧', 'Tanah': '⛰️', 'Kayu': '🌿', 'Logam': '⚜️', 'Udara': '🌬️'
  };
  return icons[element] || '✨';
}

// Fungsi dapatkan warna elemen
function getElementColor(element) {
  const colors = {
    'Api': '#FF6B6B', 'Air': '#4ECDC4', 'Tanah': '#C7B198', 'Kayu': '#96C93D', 'Logam': '#9B59B6', 'Udara': '#BDC3C7'
  };
  return colors[element] || '#D4AF37';
}

// ========== FUNGSI UTAMA loadNameOptions ==========
async function loadNameOptions(packageType) {
  console.log('🚀 loadNameOptions dipanggil untuk:', packageType);
  
  // 1. Validasi container
  const container = document.getElementById('nameOptionsContainer');
  if (!container) {
    console.error('❌ Container nameOptionsContainer tidak ditemukan');
    showErrorToast('Container tidak ditemukan. Refresh halaman.');
    return;
  }
  ...
  const originalName = fullNameInput?.value?.trim() || userNameInput?.value?.trim() || 'Sang Pencari';
  const baseName = originalName.split(' ')[0];
  
  const currentUserData = window.currentUserData;
  if (!currentUserData || !currentUserData.weton) {
    console.error('❌ currentUserData tidak tersedia');
    showErrorToast('Silakan isi tanggal lahir dan klik "SINGKAP RAMALAN" terlebih dahulu.');
    return;
  }
  
  // 3. Konfigurasi target per paket
  const targetRanges = {
  ...
  // 4. Tampilkan loading
  container.innerHTML = `
    <div style="text-align: center; padding: 40px 20px; background: rgba(15,15,20,0.9); border-radius: 20px;">
      <i class="fas fa-spinner fa-spin" style="font-size: 3rem; color: ${packageColors[packageType]}; margin-bottom: 16px;"></i>
      <p style="color: #E8E8E8; font-size: 1.1rem; margin-bottom: 8px;">
        <i class="fas fa-magic"></i> Mencari nama selaras...
      </p>
      <p style="color: #A0A0B0; font-size: 0.9rem;">
        Nama dasar: <strong>${baseName}</strong> | Weton: ${currentUserData.weton.combo} | Target: ${range.min}-${range.max}%
      </p>
    </div>
  `;
  ...
  let data;
  try {
    data = await loadNamaSelarasData();
  } catch (error) {
    console.error('❌ Gagal load data:', error);
    container.innerHTML = `
      <div style="text-align: center; padding: 30px; background: rgba(255,100,100,0.1); border-radius: 16px; border: 1px solid #FF8A8A;">
        <i class="fas fa-exclamation-circle" style="color: #FF8A8A; font-size: 2rem;"></i>
        <p style="color: #FF8A8A; margin-top: 16px;">Gagal memuat data nama. Silakan refresh halaman.</p>
      </div>
    `;
    return;
  }
  
  // 6. Generate 3 opsi nama
  const options = [];
  const seenNames = new Set();
  const wetonNeptu = currentUserData.weton.neptu || 10;
  const wukuValue = currentUserData.wuku?.value || 5;
  ...
  while (options.length < 3 && attempts < maxAttempts) {
    ...
    // Pilih random prefix dan suffix
    ...
    let newName, modType, comboType;
    
    // Variasi kombinasi
    const variation = options.length;
    
    if (variation === 0) {
      // Kombinasi 1: Prefix + Nama Asli
      newName = `${prefix.text} ${baseName}`;
      modType = `Prefix "${prefix.text}" (${prefix.era}) ditambahkan di depan nama asli`;
      comboType = 'prefix-only';
    } else if (variation === 1) {
      // Kombinasi 2: Nama Asli + Suffix
      newName = `${baseName} ${suffix.text}`;
      modType = `Suffix "${suffix.text}" (${suffix.era}) ditambahkan di belakang nama asli`;
      comboType = 'suffix-only';
    } else {
      // Kombinasi 3: Prefix + Suffix (nama baru sepenuhnya)
      newName = `${prefix.text} ${suffix.text}`;
      modType = `Nama baru: ${prefix.text} + ${suffix.text} (${prefix.era} & ${suffix.era})`;
      comboType = 'full';
    }
    
    // Skip jika sudah ada
    if (seenNames.has(newName)) continue;
    seenNames.add(newName);
    
    // Hitung kekuatan
    let hanacarakaTotal;
    if (comboType === 'prefix-only') {
      const baseValue = typeof hanacarakaValue === 'function' ? hanacarakaValue(baseName).total : baseName.length * 5;
      hanacarakaTotal = baseValue + (prefix.hanacaraka_value || 20);
    } else if (comboType === 'suffix-only') {
      const baseValue = typeof hanacarakaValue === 'function' ? hanacarakaValue(baseName).total : baseName.length * 5;
      hanacarakaTotal = baseValue + (suffix.hanacaraka_value || 20);
    } else {
      hanacarakaTotal = (prefix.hanacaraka_value || 20) + (suffix.hanacaraka_value || 20);
    }
    
    let strength = calculateNameStrengthFromParts(
      comboType === 'prefix-only' ? prefix.hanacaraka_value : (prefix.hanacaraka_value || 20),
      comboType === 'suffix-only' ? suffix.hanacaraka_value : (suffix.hanacaraka_value || 20),
      wetonNeptu,
      wukuValue
    );
    
    // Sesuaikan dengan range target
    if (strength < range.min) {
      strength = range.min + Math.floor(Math.random() * 5);
    } else if (strength > range.max) {
      strength = range.max - Math.floor(Math.random() * 5);
    }
    
    const strengthLabel = getStrengthLabel(strength);
    
    // Tentukan elemen kombinasi
    let elementCombo;
    if (comboType === 'prefix-only') {
      elementCombo = prefix.element;
    } else if (comboType === 'suffix-only') {
      elementCombo = suffix.element;
    } else {
      elementCombo = `${prefix.element}-${suffix.element}`;
    }
    ...
    options.push({
      ...
      strengthStars: strengthLabel.stars,
      prefix: prefix,
      suffix: suffix,
      ...
      era: comboType === 'full' ? `${prefix.era}-${suffix.era}` : (comboType === 'prefix-only' ? prefix.era : suffix.era)
    });
  }
  
  // Jika kurang dari 3, tambahkan fallback
  if (options.length < 3) {
    console.warn('⚠️ Kurang dari 3 opsi, menambahkan fallback...');
    const fallbackPrefixes = data.prefixes.slice(0, 5);
    const fallbackSuffixes = data.suffixes.slice(0, 5);
    
    for (let i = options.length; i < 3; i++) {
      const prefix = fallbackPrefixes[i % fallbackPrefixes.length];
      const suffix = fallbackSuffixes[i % fallbackSuffixes.length];
      const newName = `${prefix.text} ${suffix.text}`;
      
      if (seenNames.has(newName)) continue;
      seenNames.add(newName);
      
      const strength = range.min + Math.floor(Math.random() * (range.max - range.min));
      const strengthLabel = getStrengthLabel(strength);
      ...
      options.push({
        ...
        prefix: prefix,
        suffix: suffix,
        modificationType: `Fallback: ${prefix.text} + ${suffix.text}`,
        comboType: 'full',
        element: `${prefix.element}-${suffix.element}`,
        hanacarakaTotal: (prefix.hanacaraka_value || 20) + (suffix.hanacaraka_value || 20),
        era: `${prefix.era}-${suffix.era}`
      });
    }
  }
  
  // Urutkan berdasarkan kekuatan (tertinggi ke terendah)
  options.sort((a, b) => b.strength - a.strength);
  
  // 7. Simpan data untuk purchase nanti
  window.pendingNameOptions = {
    options: options,
    packageType: packageType,
    originalName: originalName,
    baseName: baseName,
    weton: currentUserData.weton,
    wuku: currentUserData.wuku
  };
  
  // 8. Render hasil
  const packageColor = packageColors[packageType];
  const packageName = packageNames[packageType];
  
  let html = `
    <div style="margin-top: 24px; animation: fadeIn 0.5s ease;">
      
      <!-- Header -->
      <div style="text-align: center; margin-bottom: 24px;">
        <h4 style="color: ${packageColor}; font-size: 1.5rem; margin-bottom: 8px; display: flex; align-items: center; justify-content: center; gap: 10px;">
          <i class="fas fa-crown"></i> 
          Paket ${packageName}
          <i class="fas fa-crown"></i>
        </h4>
        <p style="color: #A0A0B0; margin-bottom: 8px;">
          <i class="fas fa-user"></i> ${originalName} • 
          <i class="fas fa-calendar"></i> ${currentUserData.weton.combo} • 
          <i class="fas fa-bullseye"></i> Target: ${range.min}-${range.max}%
        </p>
        <p style="color: #64C864; font-size: 0.9rem;">
          <i class="fas fa-gift"></i> ${options.length} opsi nama selaras siap dipilih!
        </p>
      </div>
  `;
  ...
  options.forEach((opt, index) => {
    const strengthColor = opt.strength >= 90 ? '#64C864' : opt.strength >= 75 ? '#D4AF37' : '#FFA500';
    const borderGlow = opt.strength >= 90 ? '0 0 20px rgba(100,200,100,0.3)' : opt.strength >= 75 ? '0 0 20px rgba(212,175,55,0.3)' : 'none';
    
    html += `
      <div style="
        background: linear-gradient(145deg, rgba(20,20,25,0.98), rgba(10,10,12,0.98));
        border-radius: 20px;
        padding: 24px;
        margin-bottom: 20px;
      ...
      " onmouseover="this.style.transform='scale(1.01)'" onmouseout="this.style.transform='scale(1)'">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
          ...
          <span style="color: #A0A0B0; font-size: 0.8rem;">
            <i class="fas fa-hashtag"></i> Hanacaraka: ${opt.hanacarakaTotal}
          </span>
        </div>
        ...
        <div style="text-align: center; margin-bottom: 16px;">
          ...
          <div style="display: flex; justify-content: center; gap: 20px; margin-bottom: 8px;">
            <span style="color: ${getElementColor(opt.prefix.element)};">
              ${getElementIcon(opt.prefix.element)} ${opt.prefix.element}
            </span>
            ${opt.comboType === 'full' ? `
              <span style="color: ${getElementColor(opt.suffix.element)};">
                ${getElementIcon(opt.suffix.element)} ${opt.suffix.element}
              </span>
            ` : ''}
          </div>
          
          <div style="font-size: 2.8rem; font-weight: bold; color: ${strengthColor}; line-height: 1.2;">
            ${opt.strength}%
          </div>
          <div style="color: ${strengthColor}; font-size: 1.1rem; margin-top: 4px;">
            ${opt.strengthLabel} ${opt.strengthStars}
          </div>
        </div>
        
        <!-- Info Prefix & Suffix -->
        <div style="background: rgba(15,15,20,0.6); border-radius: 12px; padding: 16px; margin-bottom: 16px;">
    `;
    
    // Info Prefix
    html += `
      <div style="margin-bottom: 12px;">
        <p style="color: #D4AF37; margin-bottom: 4px;">
          <i class="fas fa-tag"></i> <strong>${opt.prefix.text}</strong> 
          <span style="color: #A0A0B0; font-size: 0.8rem;">(${opt.prefix.era})</span>
        </p>
        <p style="color: #B0B0C0; font-size: 0.9rem;">📖 ${opt.prefix.meaning}</p>
        <p style="color: #A0A0B0; font-size: 0.85rem;">
          ${getElementIcon(opt.prefix.element)} Elemen: ${opt.prefix.element} | 
          <i class="fas fa-bolt"></i> Energi: ${opt.prefix.energy || 'Kekuatan positif'}
        </p>
      </div>
    `;
    ...
    html += `
      <div>
        <p style="color: #D4AF37; margin-bottom: 4px;">
          <i class="fas fa-tag"></i> <strong>${opt.suffix.text}</strong>
          <span style="color: #A0A0B0; font-size: 0.8rem;">(${opt.suffix.era})</span>
        </p>
        <p style="color: #B0B0C0; font-size: 0.9rem;">📖 ${opt.suffix.meaning}</p>
        <p style="color: #A0A0B0; font-size: 0.85rem;">
          ${getElementIcon(opt.suffix.element)} Elemen: ${opt.suffix.element} | 
          <i class="fas fa-bolt"></i> Energi: ${opt.suffix.energy || 'Kekuatan positif'}
        </p>
      </div>
    </div>
    `;
    ...
    html += `
      <div style="background: rgba(212,175,55,0.1); border-radius: 8px; padding: 12px; margin-bottom: 16px;">
        <p style="color: #D4AF37; font-size: 0.9rem;">
          <i class="fas fa-magic"></i> <strong>Modifikasi:</strong> ${opt.modificationType}
        </p>
      </div>
    `;
    ...
    // Tombol
    html += `
      <button onclick="activateSelectedName('${opt.name.replace(/'/g, "\\'")}', ${opt.strength}, '${packageType}')" 
              style="
                width: 100%;
                padding: 14px;
                background: ${strengthColor};
        ...
        <i class="fas fa-check-circle"></i> AKTIFKAN NAMA INI
      </button>
    </div>
    `;
  });
  ...
  html += `
    <div style="
      ...
      border-radius: 16px;
      text-align: center;
      border: 1px dashed #64C864;
    ">
      <p style="color: #64C864; margin-bottom: 8px;">
        <i class="fas fa-heart"></i> 
        <strong>Nama yang diaktifkan akan langsung digunakan untuk analisis selanjutnya!</strong>
      </p>
      <p style="color: #A0A0B0; font-size: 0.85rem;">
        Tidak perlu mengubah dokumen legal. Energi nama bekerja saat diucapkan dan didengar.
      </p>
    </div>
  </div>
  ...
  showSuccessToast(`✨ ${options.length} opsi nama ${packageName} berhasil digenerate!`);
  
  console.log('✅ loadNameOptions selesai,', options.length, 'opsi dihasilkan');
}

// ========== FUNGSI AKTIVASI NAMA ==========
function activateSelectedName(selectedName, strength, packageType) {
  console.log('🔵 activateSelectedName:', selectedName, strength);
  
  // Update input nama lengkap
  const fullNameInput = document.getElementById('fullName');
  if (fullNameInput) {
  ...
  // Tampilkan toast sukses
  showSuccessToast(`✅ Nama "${selectedName}" telah diaktifkan! Klik "SINGKAP RAMALAN" untuk melihat hasil baru.`);
  
  // Highlight input
  if (fullNameInput) {
    fullNameInput.style.border = '2px solid #64C864';
    fullNameInput.style.transition = 'border 0.3s';
    setTimeout(() => {
      fullNameInput.style.border = '';
    }, 2000);
  }
  
  // Scroll ke tombol reveal
  const revealBtn = document.getElementById('revealBtn');
  if (revealBtn) {
    setTimeout(() => {
      revealBtn.scrollIntoView({ behavior: 'smooth', block: 'center' });
      revealBtn.style.animation = 'pulse 1s';
      setTimeout(() => revealBtn.style.animation = '', 1000);
    }, 300);
  }
}
