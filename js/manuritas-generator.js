...
// Versi: 4.0.1 - Force override untuk fix infinite recursion

// ---------- FORCE OVERRIDE TOAST FUNCTIONS ----------
// Fix untuk masalah Maximum call stack size exceeded
(function() {
    // Override showSuccessToast
...
// ... (sisa kode sama seperti versi 4.0.0 yang sudah kamu kirim)
// Mulai dari fungsi getTraitsForName sampai akhir file

// ---------- TRAITS FUNCTIONS (DIPINDAHKAN DARI RAMALAN.HTML) ----------
// ========== TRAITS FUNCTIONS (DENGAN ANTI-DUPLIKAT) ==========
function getTraitsForName(name, strengthScore, weton, wuku, filterType, limit = 2) {
  ...
  // ✅ Hilangkan duplikat berdasarkan teks
...
function generateSingleNameOption(originalName, targetMin, targetMax, weton, wuku) {
  ...
  if (powerWords.length === 0) {
    console.warn('⚠️ positiveSyllablesData kosong, menggunakan default');
  ...
  // Kombinasi 2 kata untuk Platinum
  if (targetMin >= 85) {
  ...
  // Jika tidak ada opsi valid, gunakan yang paling mendekati
  if (validOptions.length === 0) {
    console.warn('⚠️ Tidak ada opsi dalam rentang target, mencari yang terdekat...');
    ...
    combinations.forEach(combo => {
      ...
      const diff = Math.min(
        Math.abs(strength.score - targetMin),
        Math.abs(strength.score - targetMax)
      );
    ...
    if (closestOption) {
      console.log(`📊 Menggunakan opsi terdekat: ${closestOption.name} (${closestOption.strength}%)`);
...
// ---------- LOAD OPSI NAMA (DENGAN DEBUGGING) ----------
async function loadNameOptions(packageType) {
  ...
  if (!window.currentUserData) {
    alert('Data belum lengkap. Silakan refresh halaman atau klik SINGKAP RAMALAN terlebih dahulu.');
  ...
  console.log('✅ Token dipotong, sisa:', newBalance);
  ...
  try {
    ...
    // Cek positiveSyllablesData
    console.log('📊 positiveSyllablesData tersedia?', !!window.positiveSyllablesData);
    ...
    console.log('📊 Opsi yang dihasilkan:', option);
    ...
    console.log('✅ displayNameOptionInline selesai');
    
  } catch (e) {
    console.error('❌ Gagal generate opsi:', e);
    alert('Terjadi kesalahan saat generate opsi nama: ' + e.message);
  } finally {
...
// ========== GENERATE SUMMARY UNTUK NAMA BARU ==========
function generateSummaryForNewName(data, result, kelebihan, kekurangan) {
  ...
  // Paragraf 3
  const kelebihanText = uniqueKelebihan.length > 0 
    ? uniqueKelebihan.join(' ') 
    : 'energi yang seimbang dan harmonis';
  const kekuranganText = uniqueKekurangan.length > 0 
    ? uniqueKekurangan.join(' ') 
